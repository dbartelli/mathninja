...
- **Skip the easy ones** — in Single Number Power Mode, optionally exclude the trivial `×0, ×1` facts and/or the shortcut `×2, ×10, ×11` facts (division facts built from the same numbers are skipped too) so practice time goes to the facts still being learned
...
There's no build step or server-side code — reload the tab after each edit to pick up changes.
